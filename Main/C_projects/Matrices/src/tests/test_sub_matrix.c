...
START_TEST(test_sub_matrix_loop) {
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t otvet = {0};
  matrix_t res = {0};
  ...
  int err = s21_sub_matrix(&A, &B, &res);
...
START_TEST(test_sub_matrix_error_diff_rows_cols) {
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t otvet = {0};
  matrix_t res = {0};
...
START_TEST(test_sub_matrix_error_nan) {
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t otvet = {0};
  matrix_t res = {0};
...
Suite *s21_test_sub_matrix_suite(void) {
  Suite *s;
  TCase *tc_core;
  s = suite_create("test_s21_sub_matrix");
  tc_core = tcase_create("Core");

  suite_add_tcase(s, tc_core);

  tcase_add_loop_test(tc_core, test_sub_matrix_loop, 1, 100);
  tcase_add_test(tc_core, test_sub_matrix_error_nan);
  tcase_add_test(tc_core, test_sub_matrix_error_diff_rows_cols);
